...
- Ria Manu Joseph (24JE0683)
	- [Project 1] Implemented user-accessible semaphore syscalls (sem_init, sem_down, sem_up) and validated behavior via semtest.
...
- Samriddhi Singh (24JE0685)
	- [Project 1] Modified sendmsg and recvmsg syscalls with per-process message buffer/flag handling and validated via msgtest (parent-child messaging with fork/sendmsg/recvmsg polling).
	- [Project 2] Implemented sh_grep with flags (-n, -i, -v, -c), stdin support, and error handling.

- Sandeep Kumar (24JE0686)
	- [Project 1] Enhanced kill syscall into multi-mode behavior (normal, immediate, delayed) with kernel-side process state support and dummy-based testing.
...
- Sangareddygari Ashish Varun Reddy (24JE0687)
	- [Project 1] Implemented poweroff syscall and user utility to terminate processes, flush logs, and trigger QEMU shutdown.
	- [Project 2] Implemented sh_wc utility supporting line, word, and byte counting with flag-based output.

- Sanjib Behera (24JE0688)
	- [Project 1] Implemented getpinfo() syscall and ps user process integration to print process snapshot.
